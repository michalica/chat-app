...
export async function getAllMessages(
  messagesCallback: (messages: MessageStructure[]) => void
): Promise<void> {
  try {
    messagesCallback(
      await fetchFrom<MessageStructure[]>(endpoints.allMessage())
    );
  } catch (e) {
...
export async function sendMessage(
  ...
  try {
    await postTo<MessageStructure[]>(endpoints.sendMessage(), data);
