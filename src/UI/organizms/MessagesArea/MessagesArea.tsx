    ...
    <div className={classes.root}>
      {messages.map((message: MessageStructure, index: number) => {
        if (message.author === myUserName) {
          return (
            <div key={index} className={classes.myMessageWrapper}>
              <CurrentUserMessageBox messageDetails={message} />
            </div>
          );
        }
        ...
        return (
          <div key={index} className={classes.messageWrapper}>
            <MessageBox messageDetails={message} />
          </div>
        );
      })}
      <div style={{ float: "left", clear: "both" }} ref={lastDiv} />
    </div>
  );
};
